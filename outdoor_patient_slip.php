  ...
  if (isset($_POST['outdoor-patient-slip-submit'])) {
    ...
    if (!mysqli_stmt_prepare($stmt,$sql)) {
        ...
        if ($resultCheck > 0) {
          ...
          $slipQuery = "INSERT INTO `outdoor_slip`(`SLIP_MR_ID`, `DEPT_ID`, `DOCTOR_ID`, `SLIP_FEE`, `SLIP_DATE_TIME`, `STAFF_ID`) VALUES (?,?,?,?,?,?)";
          mysqli_stmt_execute($stmt);
            ...
            if (!mysqli_stmt_prepare($stmt,$slipQuery)) {
              ...
              mysqli_stmt_bind_param($stmt,"ssssss", $mrid,$dept,$doctor,$fee,$saveOn,$by);
              ...
              if (mysqli_stmt_execute($stmt)){
                ...
                if (!mysqli_stmt_prepare($stmt,$slipQuery)) {
                  ...
                  if (mysqli_stmt_execute($stmt)) {
                    ...
                    echo '<script type="text/javascript">window.location = "outdoor_slip_print.php?dept='.$dept.'&pname='.$name.'&on='.$saveOn.'&mrid='.$mrid.'&phone='.$phone.'&cnic='.$cnic.'&gender='.$gender.'&doc='.$doctor.'&age='.$age.'&add='.$address.'&fee='.$fee.'&by='.$by.'";</script>';
                  } 
                }   
              }
            exit();
