<?php
  ...
  if ($q == 'ADD_STANDALONE_EMERGENCY_BILL') {
    // Post Variables
    $billId = mysqli_real_escape_string($db, $_POST['billId']);
    $mrId = mysqli_real_escape_string($db, $_POST['patId']);
    $slipId = mysqli_real_escape_string($db, $_POST['slipId']);
    $name = mysqli_real_escape_string($db, $_POST['name']);
    $phone = mysqli_real_escape_string($db, $_POST['phone']);
    ...
    // Check if MR ID and Bill ID Exists
    if ($mrId && $billId) {
        // Patient Insert Query
        $patientQuery = "INSERT INTO `me_patient` 
        (`PATIENT_MR_ID`, `PATIENT_NAME`, `PATIENT_MOBILE`, `PATIENT_GENDER`, `PATIENT_AGE`, `STAFF_ID`) 
        VALUES (?,?,?,?,?,?)";

        // Initialize statement
        $stmt = mysqli_stmt_init($db);

        // Prepare and execute patient query
        if (mysqli_stmt_prepare($stmt, $patientQuery)) {
            $params = [
              $mrId, $name, $phone, $gender, $age, $staffId
            ];
            mysqli_stmt_bind_param($stmt, str_repeat("s", count($params)), ...$params);
            if (mysqli_stmt_execute($stmt)) {
                $response['PATIENT_INSERT'] = 'SUCCESS';

                $slipQuery = "INSERT INTO `me_slip` 
                (`SLIP_UUID`, `SLIP_MRID`, `SLIP_NAME`, `SLIP_MOBILE`, `SLIP_DISPOSAL`, `SLIP_DEPARTMENT`, `SLIP_DOCTOR`, `SLIP_FEE`, `SLIP_DISCOUNT`, `SLIP_PROCEDURE`, `SLIP_TYPE`, `SLIP_SUB_TYPE`, `STAFF_ID`)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        
                // Prepare and execute slip query
                if (mysqli_stmt_prepare($stmt, $slipQuery)) {
                    $params = [ $slipId, $mrId, $name, $phone, $disposal, NULL, $doctor, $finalBill, $discount, NULL, 'EMERGENCY', NULL, $staffId];
                    mysqli_stmt_bind_param($stmt, str_repeat("s", count($params)), ...$params);
                    if (mysqli_stmt_execute($stmt)) {
                        $response['SLIP_INSERT'] = 'SUCCESS';
                        ...
                        // if($resultCheck == 0){
                          $sql = "INSERT INTO `me_bill`(
                            ...
                            if (!mysqli_stmt_prepare($stmt,$sql)) {
                              ...
                              mysqli_stmt_bind_param($stmt, "sssssssss", $billId, $mrId, $slipId, $name, $phone, $totalBill, $discount, $finalBill, $staffId);
                              if (mysqli_stmt_execute($stmt)){
                                $billQuery = "INSERT INTO `me_emergency`(
                                  `EMERGENCY_UUID`,`EMERGENCY_SLIP_UUID`,`STAFF_ID`,`ES_MO_FEE`,`INJECTION_IM`, 
                                  `INJECTION_IV`,`DRIP`,`DRIP_VENOFER`,`INFUSION_ANTIBIOTIC`, 
                                  `IV_LINE_IN_OUT`,`DRESSING_SMALL_LARGE`,`PER_STITCH_IN`,`PER_STITCH_OUT`,
                                  `BSF_BSR`,`BP`,`NEBULIZATION`,`ECG`,`MONITOR_CHARGE`,`CTG`,`FOLEY_CATHETER`,
                                  ...
                                  if (!mysqli_stmt_prepare($stmt,$billQuery)) {
                                    $result = [];
                                    $result['status'] = "error";
                                    $result['message'] = "SQL Database Error!";
                                    echo json_encode($result);
                ...
                $response['PATIENT_INSERT'] = 'FAILURE';
                $response['MESSAGE'] = 'Error: ' . mysqli_error($db);
                echo json_encode($response);
                exit;
            }
        ...
        // if ($response['PATIENT_INSERT'] === 'SUCCESS' && $response['SLIP_INSERT'] === 'SUCCESS' && $response['BILL_INSERT'] === 'SUCCESS') {
        //     $response['RESULT'] = 'BILL_CREATED';
        // } else {
        //     $response['RESULT'] = 'BILL_NOT_CREATED';
        // }
        // echo json_encode($response);
    } 
    // else {
    //     $response['RESULT'] = 'BILL_NOT_CREATED';
    //     $response['MESSAGE'] = 'MR ID or Bill ID not provided';
    //     echo json_encode($response);
    // }
  } 
