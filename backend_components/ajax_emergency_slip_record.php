<?php
    ...
    // Add EMERGENCY-SLIP Request Query
    if ($q == 'GENERATE-EMERGENCY-SLIP') {

        $title = mysqli_real_escape_string($db, $_POST['title']);
        $comment = mysqli_real_escape_string($db, $_POST['comment']);
        $by = mysqli_real_escape_string($db, $_POST['userId']);
        $table = 'EMERGENCY_SLIP_REQUEST';
    ...
    if ($q == 'VIEW-EMERGENCY-SLIP') {
        $str = "EMERGENCY_SLIP_REQUEST";
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            <td>
                <a href='javascript:void(0);' onclick='updateEmergencyRequest($row[REQUEST_ID])' data-toggle='modal' data-target='#edit-request'>
                    <i class='fas fa-edit'></i>
                </a>
                <a onClick='deleteEmergencyRequest($row[REQUEST_ID])' href='javascript:void(0);' style='color:red;'>
