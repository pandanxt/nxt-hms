<?php
    ...
    // Add INDOOR-SLIP Request Query
    if ($q == 'GENERATE-INDOOR-SLIP') {

        $title = mysqli_real_escape_string($db, $_POST['title']);
        $comment = mysqli_real_escape_string($db, $_POST['comment']);
        $by = mysqli_real_escape_string($db, $_POST['userId']);
        $table = 'INDOOR_SLIP_REQUEST';
    ...
    if ($q == 'VIEW-INDOOR-SLIP') {
        $str = "INDOOR_SLIP_REQUEST";
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            <td>
                <a href='javascript:void(0);' onclick='updateIndoorRequest($row[REQUEST_ID])' data-toggle='modal' data-target='#edit-request'>
                    <i class='fas fa-edit'></i>
                </a>
                <a onClick='deleteIndoorRequest($row[REQUEST_ID])' href='javascript:void(0);' style='color:red;'>
