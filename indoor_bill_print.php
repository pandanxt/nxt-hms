<?php
      ...
      //******************************* //
          if ($type == "gynae" || $type == "gensurgery") {
            if ($sid){
              ...
              $billSql ="SELECT `a`.*, `b`.`ADMIN_USERNAME`	FROM `indoor_gensurgery_bill` AS `a` INNER JOIN `admin` AS `b` ON `b`.`ADMIN_ID` = `a`.`CREATED_BY` WHERE `BILL_ID` = ".$sid;
    ...
    //**********************************//
    //******General Illness Print******//
    //******************************* //  
    } else if ($type == "genillness") {

            if ($sid){
              // Query to get Slip Details 
              $billSql ="SELECT `a`.*, `b`.`ADMIN_USERNAME`	FROM `indoor_genillness_bill` AS `a` INNER JOIN `admin` AS `b` ON `b`.`ADMIN_ID` = `a`.`CREATED_BY` WHERE `BILL_ID` = ".$sid;
              $billsql = mysqli_query($db,$billSql);
              $bill_row = mysqli_fetch_array($billsql);
    ...
    //******************************* //
          ...
          if ($sid){
            // Query to get Slip Details 
            $billSql ="SELECT `a`.*, `b`.`ADMIN_USERNAME`	FROM `indoor_eye_bill` AS `a` INNER JOIN `admin` AS `b` ON `b`.`ADMIN_ID` = `a`.`CREATED_BY` WHERE `BILL_ID` = ".$sid;
            $billsql = mysqli_query($db,$billSql);
            $bill_row = mysqli_fetch_array($billsql);

            $fee = $bill_row['TOTAL_AMOUNT'];
            $discount = $bill_row['DISCOUNT'];
            $finalFee = $bill_row['TOTAL'];
            ...
            FROM `indoor_slip` AS `a` INNER JOIN `doctor` AS `b` ON `b`.`DOCTOR_ID` = `a`.`DOCTOR_ID`
            WHERE `SLIP_ID` = '$bill_row[SLIP_ID]'";
            echo "<script>console.log(".$bill_row['SLIP_ID'].");</script>";
            $slipsql = mysqli_query($db,$slipSql);
            $slip_row = mysqli_fetch_array($slipsql);
    ...
    ?>
          ...
          <?php 
              ...
              $fee = (isset($_GET['tbill']) ? $_GET['tbill'] : '');
              $discount = (isset($_GET['dis']) ? $_GET['dis'] : '');
              $finalFee = (isset($_GET['fbill']) ? $_GET['fbill'] : '');
