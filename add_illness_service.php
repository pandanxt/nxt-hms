<?php session_start(); ?>
 <!-- Header Form -->
  ...
  <!-- /.Main Sidebar Container-->
    ...
    if (isset($_POST['illness-service'])) {
        $name =  $_POST['name'];
        $status =  $_POST['status'];
        $date =  $_POST['addDate'];
        $by = $_POST['by'];

            $sql = "SELECT * FROM `illness_service` WHERE `SERVICE_NAME` = ?";
            $stmt = mysqli_stmt_init($db);
            
            if (!mysqli_stmt_prepare($stmt,$sql)) {
                // header("Location: emergency_service.php?action=sqlerror");
                echo '<script type="text/javascript">window.location = "illness_service.php?action=sqlerror";</script>';
                exit();
            }else{
                ...
                $resultCheck = mysqli_stmt_num_rows($stmt);
                    ...
                    if ($resultCheck > 0) {
                        ...
                        echo '<script type="text/javascript">window.location = "illness_service.php?action=nameTaken";</script>';
                        exit();
                    }else{
                            $sql = "INSERT INTO `illness_service` (`SERVICE_NAME`, `SERVICE_STATUS`, `STAFF_ID`, `SERVICE_DATE_TIME`) VALUES (?,?,?,?)";
                            mysqli_stmt_execute($stmt);
                            ...
                            if (!mysqli_stmt_prepare($stmt,$sql)) {
                                ...
                                echo '<script type="text/javascript">window.location = "add_illness_service.php?action=saved";</script>';								
                                exit();
                            }			
                        }
                }
      ...
      <div class="container-fluid">
        ...
        <div class="card card-info">
          ...
          <div class="card-footer" style="text-align: right;">
            <button type="submit" name="illness-service" class="btn btn-block btn-primary">Submit</button>
          </div>
        </div>
        <!-- /.card -->
        </form>
